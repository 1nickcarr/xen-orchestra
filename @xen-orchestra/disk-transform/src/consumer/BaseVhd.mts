...
export abstract class BaseVhd {
  ...
  computeVhdHeader() {
    console.log('compute vhd header')
    ...
    console.log('compute vhd header done')
    return header
  }
  computeVhdFooter(): Buffer {
    ...
    const diskType = source.parentUuid !== undefined ? DISK_TYPES.DIFFERENCING : DISK_TYPES.DYNAMIC
    const footer = createFooter(size, Math.floor(Date.now() / 1000), geometry, FOOTER_SIZE, diskType)
    console.log('compute vhd footer done')
    return footer
  }
